...
export default function NotionStyleEditor({
  ...
  const [rewriteRange, setRewriteRange] = useState<Range | null>(null);
    ...
    // If editor is empty or still the default placeholder, fill with initialHtml
    if (
      !current ||
      current === "<p><br></p>" ||
      current === "<p></p>" ||
    ...
    // In edit mode, if initialHtml changes and is different, update it
  ...
  try {
    ...
    // 1) CREATE (if in "create" mode or no id yet)
